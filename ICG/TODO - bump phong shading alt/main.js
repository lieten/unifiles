	...
	constructor ( columns, rows, width, height )
	{
		...
		for ( let y=0 ; y < numYVertices ; y++ )
		{
			for ( let x=0 ; x < numXVertices ; x++ )
			{
				...
				let distance = Math.sqrt(posX*posX + posY*posY) //Distanz vom Ursprung (von der Insel)
				let timeFactor = framesRendered //Zeitfaktor für die Wellen
				let posZ = Math.sin((distance + timeFactor)*10)*0.075 //Wellen, die kreisförmig von der Insel ausgehen
				//TODO: VBOs jedes mal updaten
